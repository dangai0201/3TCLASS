...
public class RoomJni {
    ...
    public native void RoomQuickEnter(String appId, long nUserID, long nRoomID, int userRole, String rtmpUrl, String model , boolean recordMp4 , String sCustom);
    ...
    public native void UpdateUserBaseInfo(String strCustom);
    ...
    private void OnRoomMemberEnter(long nConfID, long nUserID, String szUserXml,
        ...
        PviewLog.jniCall("OnRoomMemberEnter", "nConfID : " + nConfID + " | nUserID : " + nUserID + " | szUserXml : " + szUserXml
                + " | userRole : " + userRole + " | speakStatus : " + speakStatus + " | sCustom : " + sCustom);
        if (GlobalConfig.trunOnCallback) {
            for (int i = 0; i < this.mCallBacks.size(); i++) {
                WeakReference<PviewConferenceRequest> wf = this.mCallBacks.get(i);
                if (wf != null && wf.get() != null) {
                    wf.get().OnConfMemberEnter(nConfID, nUserID, szUserXml, userRole, speakStatus , sCustom);
                }
            }
        }
    }
...
//                + audioLevel+ " | audioLevelFullRange : " + audioLevelFullRange);
    ...
    private void OnUpdateUserBaseInfo(long roomID , long uid , String sCustom) {
        PviewLog.jniCall("OnUpdateUserBaseInfo", "roomID : " + roomID + " | uid : " + uid + " | sCustom : " + sCustom);
        if (GlobalConfig.trunOnCallback) {
            for (int i = 0; i < this.mCallBacks.size(); i++) {
                WeakReference<PviewConferenceRequest> wf = this.mCallBacks.get(i);
                if (wf != null && wf.get() != null) {
                    wf.get().OnUpdateUserBaseInfo(roomID , uid , sCustom);
                }
            }
        }
    }
