...
//    private boolean mFirstVideoSent = false;
    ...
    public int requestEnterRoom(String appId, long confId, long userId, int userRole, String rtmpUrl, boolean recordMp4, String sCustom, Handler caller) {
...
//        mFirstVideoSent = false;
        return InstantRequest.getInstance().requestServer(caller, REQUEST_JOIN_ROOM, appId, userId,
                confId, userRole, rtmpUrl, recordMp4, sCustom);
    }
    ...
    public void OnConfMemberEnter(long nConfID, long nUserID, String szDeviceInfos,
                                  int userRole, int speakStatus , String sCustom) {
        ...
        if (userRole == CLIENT_ROLE_BROADCASTER) {
            ...
            mJniWorkerThread.sendMessage(JniWorkerThread.JNI_CALL_BACK_USER_JOIN, new Object[]{nUserID, CLIENT_ROLE_BROADCASTER , sCustom});
            ...
            mJniWorkerThread.sendMessage(JniWorkerThread.JNI_CALL_BACK_USER_JOIN, new Object[]{nUserID, CLIENT_ROLE_AUDIENCE , sCustom});
        } else if (userRole == CLIENT_ROLE_ANCHOR) {
            GlobalHolder.getInstance().getUser(nUserID).setUserIdentity(Constants.CLIENT_ROLE_ANCHOR);
            mJniWorkerThread.sendMessage(JniWorkerThread.JNI_CALL_BACK_USER_JOIN, new Object[]{nUserID, CLIENT_ROLE_ANCHOR , sCustom});
            if (remoteUserDeviceInfos != null && remoteUserDeviceInfos.mInUse) {
                EnterConfApi.getInstance().openDeviceVideo(nUserID, remoteUserDeviceInfos.mUserDeviceID);
            }
        }
...
//        mFirstVideoSent = true;
    ...
    public void OnUpdateUserBaseInfo(long roomID, long uid, String sCustom) {
        JniWorkerThread mJniWorkerThread = GlobalHolder.getInstance().getWorkerThread();
        mJniWorkerThread.sendMessage(JniWorkerThread.JNI_CALL_BACK_ON_UPDATE_USER_INFO, new Object[]{roomID , uid , sCustom});
    }
    ...
    public void OnConnectSuccess(String uuid, String ip, int port) {
        ...
        JniWorkerThread mJniWorkerThread = GlobalHolder.getInstance().getWorkerThread();
        mJniWorkerThread.sendMessage(JniWorkerThread.JNI_CALL_BACK_ON_CONNECT_SUCCESS, new Object[]{ip , port});
    }
