...
public class RoomLiveHelp {
    ...
    private static final String TAG_CLASS = RoomLiveHelp.class.getSimpleName();
                ...
                switch (mJniObjs.mJniType) {
                    ...
                    case CALL_BACK_ON_USER_JOIN:
                        ...
                        if(identity == Constants.CLIENT_ROLE_BROADCASTER){
                            mLiveInstance.onMemberEnter(uid, mRemoteUserInfo , mJniObjs.sCustom);
                        }else {
                            mLiveInstance.onHostEnter(uid, mRemoteUserInfo  , mJniObjs.sCustom);
                        }
                        break;
...
//                            messageData.clear();
                    ...
                    case LocalConstans.CALL_BACK_ON_REMOTE_AUDIO_STATE:
                        ...
                        break;
                    case LocalConstans.CALL_BACK_ON_UPDATE_USER_INFO:
                        mLiveInstance.OnupdateUserBaseInfo(mJniObjs.mRoomId , mJniObjs.mUid , mJniObjs.sCustom);
                        break;
                    case LocalConstans.CALL_BACK_ON_CONNECT_SUCCESS:
                        mLiveInstance.OnConnectSuccess(mJniObjs.mIP , mJniObjs.mPort);
                        break;
    ...
    public void enterRoom(int liveType,int role,final int roomId,final long userId,int i , String sCustom) {
        Log.e(TAG_CLASS,  Constants.SDKVideo_url  + "   port    " +Constants.SDKVideo_port);
        mTTTEngine = TTTRtcEngine.getInstance();
        RoomJni.getInstance().setServerAddress(Constants.SDKVideo_url,Constants.SDKVideo_port);

//        RoomJni.getInstance().setServerAddress("47.94.253.169",5000);
//        RoomJni.getInstance().setServerAddress("101.71.142.135",5000);
        ...
        Log.e(TAG_CLASS + " 动态视频分辨率 " , "role = " + role + "  i = " + i);
        //设置视频分辨率
        videoProfile(role, i);
...
//                GlobalConfig.mPushUrl = "";
                int result = mTTTEngine.joinChannel("", String.valueOf(roomId), userId  ,sCustom);
...
//            videoConfig.videoWidth = width;
//            videoConfig.videoHeight = height;
...
//        long id = info.getId();
    ...
    public void updateUserInfo(String sCustom){
        mTTTEngine = TTTRtcEngine.getInstance();
        mTTTEngine.updateUserBaseInfo(sCustom);
    }
