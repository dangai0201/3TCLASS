...
public class WorkerThread extends Thread {
    ...
    int joinChannel(String channelKey,
        ...
        mWorkerHandler.post(new Runnable() {
            @Override
            public void run() {
                mTTTRtcEngine.joinRealChannel(channelKey, channelName, optionalUid , sCustom);
            }
        });
                ...
                case ACTION_WORKER_VIDEO_STATUS:
                    ...
                    if (GlobalConfig.mIsInRoom.get()) {
                        ...
                        if (GlobalConfig.mIsEnableVideoMode) {
                            ...
                            int drop_bytes_diff =  videoModule.getflowCtrlBytes() - mLastFlowCtrlBytes;
                            int enc_bytes_diff = videoModule.getEncodeDataSize() - mLastEncodeDatas;
                            if (enc_bytes_diff > 0) {
                                drop_bytes_rate = (float)drop_bytes_diff / (float)enc_bytes_diff;
                            }
                            LocalVideoStats stats = new LocalVideoStats(mLocalVideoSendPS, mLocalFpsPS, drop_bytes_rate, videoModule.getBufferDuration());
